...
- **v1.5.0** — `maker-council` — simulated personal board of advisors (issue #1). Eight-member bench of founders/operators with colliding lenses (Fried, Musk, Bezos, Jensen, Iger, Graham, Naval, Blakely), question-type seating with a designated dissenter, dossier-grounded takes + optional live-research pass (deep-research / watch-video / last30days), disagreement map, chair's synthesis → `/decide` handoff. Adapted from marketingskills' marketing-council; archives + custom members in `$MAKERSKILLS_CONFIG/maker-council/`.

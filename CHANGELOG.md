...
### Changed
- **`social-fetch` v0.2.0** — adds bounded Xquik Tweet and Follower Actor routes, bearer-token examples, charge controls, and secret-safe credential verification while preserving existing fallbacks.
- **`domain` v0.2.0** (#22) — registry-truth patterns adopted from [iannuttall/unclaimed](https://github.com/iannuttall/unclaimed). RDAP hardening (always send a User-Agent; 404 ≠ available — reserved/blocked/premium-unsold names also 404, so confirm via whois; IANA bootstrap caveat + direct `.io`/`.so` endpoints), whois classification ordering (registered signals before available signals — parked domains say "available for sale") + reserved-name detection, IANA-referral whois-server discovery for any TLD, new **Step 7a liveness probe** (dead/parked/live classification of taken domains → outreach + marketplace leads without scraping), new **Step 7b drop-watch** (~80-day post-expiry drop estimate, backorder guidance), a three-state AVAILABLE/TAKEN/UNKNOWN rule (timeouts are never "available"), and optional `npx unclaimed` for bulk multi-TLD sweeps.
- **`vibecad` moved out** (closes #18) — extracted with history to its own repo (`coreyhaines31/vibecad`, private for now). Its changelog entries below remain for the record.

### Added, then moved out with `vibecad` (kept for the record)
